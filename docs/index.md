...
`subagent-fleet` is a **local AI compute control plane** for Claude Code and coding agents. It maps coding subagents to the best Ollama model and machine you own, then generates LiteLLM and Claude Code-style agent configuration — with real-time health monitoring, SSE dashboards, execution tracing, and model warmup.
...
See [`examples/`](https://github.com/adityak74/subagent-fleet/tree/main/examples) for complete fleet configurations and generated output:
...
| `litellm-proxy/` | Generated LiteLLM config showing how models route to nodes |
